...
router.post('/analyze', async (req, res) => {
  try {
    ...
    // If MCP server is configured, proxy the request
    if (MCP_URL) {
      try {
        const mcpPayload = {
          jsonrpc: '2.0',
          ...
          params: {
            ...
            arguments: { symbol: symbol.toUpperCase(), timeframe, indicators },
          },
          id: Date.now(),
        };
...
router.get('/symbols', (req, res) => {
  try {
    ...
    // Get unique symbols from the user's trades
    const tradeSymbols = db
      .prepare('SELECT DISTINCT UPPER(symbol) as symbol FROM trades WHERE user_id = ? AND symbol IS NOT NULL')
      .all(userId)
      .map(r => r.symbol)
      .filter(Boolean);

    // Popular symbols as defaults
    const popular = [
    ...
    // Merge user symbols (first) + popular (deduplicated)
...
router.get('/status', async (req, res) => {
  ...
  try {
    ...
    if (healthRes.ok) {
      return res.json({ status: 'connected', message: 'MCP server is online' });
    }
